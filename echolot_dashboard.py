...
def _augment_strip_css() -> str:
    ...
    /* A lang-select közvetlenül a nav-tabok MELLETT, balra-igazítva.
       ...
       a .top-actions abszolút-pozícionált gombjai lefedték desktopon.
       Mobilon flex-wrap miatt új sorba kerül, semmi gond. */
...
def _augment_block_html(lang: str, active: str = "feed") -> str:
    ...
    tabs = [
        ...
        ("analysis", "/analysis",             t("tab.analysis", lang)),
        ("entities", "/entities",             t("tab.entities", lang)),
    ]
    tab_html = []
...
def render_trending_page(request, compute_velocity_fn, db_path,
    ...
    # kihagyjuk — a velocity mostantól published_at-ra bucketál
    # (echolot_velocity.py), így fresh DB esetén is van valódi baseline.
    rows = []
    for s in report.get("spheres", []):
